...
approved prompt fixes in `tools/pali.py`, write a handoff, and stop.
...
**Model limitations:** We are using gpt-4o-mini for batch API requests, which is cheap and efficient, but may have a limitation s for our task. Consider it and if no other options suggest switch to smarter model.
...
4. Implement approved changes in `tools/pali.py`
5. Write handoff and stop — user re-runs the batch before the next session

## Strict Scope
- **In scope:** `tools/pali.py` (PALI_SYSTEM_INSTRUCTION only), `scripts/evaluate_pali.py`, `tools/glossary.py` (expanding non-core sections like EXTENDED_TERMS or new sections)
- **Out of scope:** Batch infrastructure, chunking logic, extract stage,
  core VINAYA, DHAMMA, SANGHA lists in glossary.py (unless adding to other sections), correct_pali.py internals
...
- Verification runs (next session's job after user re-runs batch)
