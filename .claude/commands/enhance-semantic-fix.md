...
**Delegation Opportunity (B10):** The context-grep step is pure retrieval (grep for pattern, return lines with context). If doing this manually for many findings would be repetitive, you may delegate it to a fast/simple sub-agent — hand it the list of findings and the folder path, ask it to grep each finding and return the surrounding context. **Use the Agent tool with `model: "haiku"` explicitly** (pure grep retrieval does not need a strong model; omitting the override defaults to the strong model and is far more expensive). The sub-agent runs with the Context Discipline Rule (no raw transcripts in the main agent's context — only the focused grep output returns to you for classification). This keeps the main agent's context light while delegating mechanical retrieval work.
