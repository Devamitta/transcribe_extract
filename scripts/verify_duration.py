...
def main():
    parser = argparse.ArgumentParser(description="Verify transcription duration against audio duration.")
    parser.add_argument("--audio-dir", type=str, required=True, help="Directory containing original MP3 files")
    parser.add_argument("--transcript-dir", type=str, required=True, help="Directory containing MD transcripts")
    parser.add_argument("--threshold", type=float, default=120.0, help="Threshold in seconds to flag truncation (default: 120s)")
    ...
    print(f"Checking {len(transcript_files)} transcripts against audio in '{audio_dir}'")
    ...
    for transcript_path in transcript_files:
        ...
        if not audio_path.exists():
            print(f"{transcript_path.name:<50} | MISSING    | {'N/A':<8} | {'N/A':<8} | Audio not found")
            continue
        ...
        if audio_duration is None or last_ts_mins is None:
            print(f"{transcript_path.name:<50} | ERROR      | {'N/A':<8} | {'N/A':<8} | Could not parse")
            continue
        ...
        print(f"{transcript_path.name[:50]:<50} | {status:<10} | {audio_str:<8} | {md_str:<8} | {diff_str}")
