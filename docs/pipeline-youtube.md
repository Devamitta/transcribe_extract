...
`--dry-run [stub_file]` runs the full pipeline without any real processing, API calls, provider diagnostics, or file mutations. Every stage prints its configured `input → output` paths for verification.
...
For manual OpenRouter image-path troubleshooting, run:

```bash
uv run python scripts/check_keys.py --image
```

Then run thumbnail generation normally:
