...
# Confirmed-by-reproduction false corrections: the LLM proposes these despite
# Rule 17's explicit prohibition, so they are blocked deterministically too.
PALI_DENY_PHRASES: tuple[str, ...] = (
    "a nature",
    "in a niche",
    "we got a two car",
)
...
def is_denied_correction(original: str) -> bool:
    """True if a proposed correction's original text matches a known false-positive phrase."""
    normalized = original.lower()
    return any(phrase in normalized for phrase in PALI_DENY_PHRASES)
...
def get_pali_system_instruction(file_path: Path) -> str:
    ...
    instruction = (
        ...
        "   Examples:\n"
        f"{semantic_examples}"
        "   Guard: only flag an English phrase as a hidden Pali term when it reads as an implausible non-sequitur in context. A standard, fluent English Buddhist phrase the teacher could have intentionally said (e.g., 'causes and conditions', a descriptive use of 'nature') is NOT evidence of a hallucination — do not translate it into a Pali equivalent (e.g., 'causes and conditions' -> 'paccaya', 'nature' -> 'anicca') just because the topic is thematically related. Only correct when the surrounding phonetics specifically garble that Pali word. Concrete case: in a discussion of anicca/dukkha, phrases like 'a nature', 'in a niche', or 'we got a two car' are the speaker's own loose English phrasing, not phonetic garbles of 'anicca'/'dukkha' — leave them unchanged even though the topic is thematically anicca/dukkha.\n"
        ...
        "12. THERAVADA PALI BIAS & GARBLE ABSORPTION: Prioritize common Theravada Pali terms over obscure or Sanskrit terms. If an English word is clearly part of a phonetic garble for a single Pali term (e.g., 'India Asambara'), ABSORB the English word into the correction (e.g., 'India Asambara' -> 'Indriyasaṃvara', NOT 'India Indriyasaṃvara'). Do not invent complex compounds if a simpler glossary term fits the phonetic footprint (e.g., 'samiltonica' -> 'Saṃyutta Nikāya'). FULL PHONETIC COVERAGE REQUIRED: before proposing any correction, verify that EVERY word in the original phrase has a phonetic correspondence in the proposed term — not just one recognizable fragment (e.g., matching only 'Sangha' in 'Oral Sangha'). If only a fragment matches and the rest of the proposed term has no phonetic support in the original phrase, do NOT propose the correction — leave the text unchanged rather than forcing a partial match into an elaborate or thematically-convenient compound. Concrete case: 'Oral Sangha' (in a Pāṭimokkha rule-counting context) has phonetic support only for 'Sangha' — do NOT correct it to an invented compound like 'Āraññaka Saṅgha' or 'uruvelasaṅghā'; leave it unchanged.\n"
        ...
        "16. AMBIGUOUS COMPETING CANDIDATES: When a garbled phrase has more than one individually phonetically-plausible Pali reading, and those readings imply different, mutually exclusive doctrinal content, use the surrounding sequence/discussion (the sentence before and after) to judge which candidate the context actually supports — do not pick the first or most common-sounding option by phonetics alone. If the surrounding context does not clearly favor one candidate over the others, do NOT propose any of them — leave the text unchanged rather than guessing. Concrete case: 'practice for half a year only on the city of dana' (discussing the bojjhaṅga sequence sati -> dhammavicaya) has drawn unstable guesses across runs ('citta or dāna', 'sati of dāna', 'cittānupassanā') with no single reading clearly fitting the doctrinal sequence — leave it unchanged.\n"
        "17. DO NOT TRANSLATE ORDINARY ENGLISH DESCRIPTION INTO PALI: when a speaker uses a plain English descriptive phrase to talk ABOUT a Pali concept (rather than mishearing the Pali word itself), leave the English as-is — do NOT replace it with the Pali term just because it is thematically adjacent. This is a stricter, mandatory version of the Rule 7 guard: DO NOT correct 'a nature' or 'in a niche' to 'anicca', and do NOT correct 'we got a two car' to 'dukkha', anywhere in the text, even in a section that explicitly discusses anicca or dukkha and even if other instances of 'anicca'/'dukkha' nearby are spelled correctly already. These exact phrases are confirmed NOT to be phonetic garbles of those Pali words; they are the speaker's own English phrasing.\n"
...
def get_semantic_eval_instruction() -> str:
    ...
    return (
        ...
        '  {"passage": "exact verbatim quote from the text", "context": "the complete spoken segment containing the passage — all text from the nearest preceding [N.N] Whisper timestamp marker to the next [N.N] marker (typically 5–15 sentences). Quote verbatim from the text.", '
